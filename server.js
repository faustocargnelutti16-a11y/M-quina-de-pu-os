...
(function () {
  ...
  return '<div class="seccion">' +
    ...
    /* QUE ARCHIVO ESTA CORRIENDO LA TELE. Una pagina abierta no se entera de
       que subiste una version nueva: sigue con la que cargo la ultima vez.
       Eso hizo que varios arreglos parecieran no funcionar. Ahora se ve. */
    'Versi\u00f3n del t\u00f3tem \u00b7 <b>' + (t.version || 'no la dice todav\u00eda') + '</b>' +
    (t.version ? '' : ' <span class="tenue">(archivo viejo)</span>') +
    '</div>' +
    '<div class="botones" style="margin-top:10px">' +
    '<a class="b" href="/api/totem/recargar' + (CLAVE ? ('?clave=' + encodeURIComponent(CLAVE)) : '') +
    '">Recargar el t\u00f3tem</a></div>' +
    '<p class="lectura tenue" style="margin-top:8px">Si subiste un archivo nuevo y la versi\u00f3n de ' +
    'arriba no es la que esper\u00e1s, la tele todav\u00eda est\u00e1 mostrando el anterior. Toc\u00e1 ' +
    '<b>Recargar el t\u00f3tem</b> y en unos segundos toma el nuevo, sin ir hasta el bar. ' +
    'Si hay una pi\u00f1a en pantalla espera a que termine la animaci\u00f3n.</p>' +
    /* El boton que faltaba: subir un archivo nuevo no cambia lo que la tele
       esta mostrando, porque sigue corriendo lo que cargo la ultima vez.
       Sin esto habia que ir hasta el bar a recargarla a mano. */
    '<div class="botones" style="margin-top:10px">' +
    '<button class="b" id="bRecargar" type="button">Recargar la pantalla del bar</button>' +
    '</div>' +
    '<script>' +
    'document.getElementById("bRecargar").onclick=function(){' +
    ' var b=this; b.textContent="Mandando...";' +
    ' fetch("/api/recargar' + c + '",{method:"POST"}).then(function(r){return r.json();})' +
    '  .then(function(j){ b.textContent = j && j.ok' +
    '     ? ("Listo - " + j.pantallas + " pantalla(s) recargando")' +
    '     : "No se pudo (clave?)"; })' +
    '  .catch(function(){ b.textContent="No se pudo"; });' +
    '};' +
    '<\/script>' +
